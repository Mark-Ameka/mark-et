    ...
    <div class="relative">
        ...
        <div class="absolute -bottom-32 left-7">
            <form method="POST" action="{{ route('user.update_profile_pic', $user->id) }}" enctype="multipart/form-data">
                ...
                <div class="flex gap-4 items-center flex-grow">
                    ...
                    <div class="flex flex-col mt-11 gap-3">
                        <div>
                            <h1 class="text-white sm:text-4xl text-2xl font-bold">{{ $user->fname }} {{ $user->lname }}</h1>
                            <h1 class="text-white">{{ $user->email }}</h1>
                        </div>
                        <div class="flex gap-2">
                            <button id="show-button" type="submit" disabled class="bg-neutral-700 enabled:bg-pink-900 shadow-lg enabled:hover:bg-pink-800  text-white font-bold md:rounded rounded-full">
            ...
            <form method="POST" action="{{ route('user.remove_profile_pic', $user->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <button type="submit" class="bg-red-900 shadow-lg hover:bg-red-800 py-2 px-2 text-white font-bold md:rounded rounded-full">
